...
#' the parameters to the double logistic equation are K1, K2, N0, and r1, r2.
#' @param k1       The carrying capacity on carbon source 1.
#' @param k2       The carrying capacity on carbon source 2 alone.
#' @param n0      The initial population size (absorbance or individuals)
#' @param r1       The exponential "growth rate" on carbon source 1.
#' @param r2       The exponential "growth rate" on carbon source 2.
#' @param t       The time at which you want to know N
#' @return        The number of cells, or N, at time t
#' @export
NAtT <- function(k1, k2, n0, r1, r2, t) {
  return( (k1 / (1 + ((k1 - n0) / n0) * exp(-r1 * t))) + (k2 / (1 + ((k2 - k1) / k1) * exp(-r2 * t))) )
}
...
#' @return        The number of cells, or N, at time t
##SlopeAtT <- function(k1, k2, n0, r1, r2, t) {
##  n <- NAtT(k1, k2, n0, r1, r2, t)
##  return(r * n * (k - n) / k)
##}
...
#' Area Under the Double Logistic Curve
#'
...
#' into a single value.
#' @param k1       The carrying capacity on carbon source 1.
#' @param k2       The carrying capacity.
#' @param n0      The initial population size
...
AreaUnderCurve <- function(k1, k2, n0, r1, r2, t_min = 0, t_max) {
  auc_l <- stats::integrate(function(x) NAtT(k1, k2, n0, r1, r2, x), t_min, t_max)
  return(auc_l)
}
