...
FitLogisticSum <- function(data_t, data_n) {
    ...
    k1_init <- max(data_n) ## first plateau is 1/2 of max
    ...
    suppressWarnings(
        nls_mod <- tryCatch(
            minpack.lm::nlsLM(n ~ 1*(k1 / (1 + ( (k1 - n0) / n0) * exp(-r1 * t))) +
                                   0*(k2 / (1 + ( (k2 - k1) / k1) * exp(-r2 * t))),
                              ...
                              lower = c(0, stats::median(data_n), 0, 0, 0),
                              upper = c(Inf, Inf, max(data_n), Inf,Inf),
                              data = d),
            error = function(e) {
