...
I first creates a directory structure to analyze breseq output for Zack's
...
def check_for_secondary_mutations_and_contamination(pathdict,pop_clone_labels,outdir):
    ...
    print(sorted(list(set(pop_clone_labels.Name))))
    print(sorted(list(set([x for x in pathdict]))))
...
def contamination_check(projdir,outdir):
    '''
...
def write_evolved_mutations(pop_clone_labels, inputdir):
    '''
    currently a hack. print out a csv file, and pipe or copy into a file.

    '''
    print('Clone,Mutation,Environment,Population')
    for f in [x for x in listdir(inputdir) if x.endswith('.gd')]:
        full_f = join(inputdir,f)
        fh = open(full_f)
        name = f.split('.')[0]
        my_row = pop_clone_labels[pop_clone_labels.Name == name].iloc[0]
        env = my_row['Environment']
        population = my_row['Population Label']
        gd = gdparse.GDParser(fh)
        muts = gd.data['mutation']
        muttype = ''
        for m in muts:
            if muts[m]['type'] == 'SNP':
                muttype = muts[m]['snp_type'].decode('utf8')
            else:
                muttype = muts[m]['type']
            print(','.join([name,muttype,env,population]))

def main():

    projdir = "/Users/Rohandinho/Box Sync/DM0-evolution"
    resultsdir = join(projdir,"results/genome-analysis")
    
    ## run the old main function. note how other functions depend on this function call.
    contamination_check(projdir, resultsdir)

    pop_clone_label_f = join(projdir,"data/rohan-formatted","populations-and-clones.csv")
    all_pop_clone_labels = pd.read_csv(pop_clone_label_f)

    ## TODO: currently I copied the evolved and subtracted genomes by hand into the inputdir.
    evolved_subtract_dir = join(resultsdir,"all_evolved_diffs")

    ## usage:
    ## python deal-with-gdiffs.py > ../results/genome-analysis/mutation_types.csv

    
    ## uncomment next line!
    #write_evolved_mutations(all_pop_clone_labels, evolved_subtract_dir)
