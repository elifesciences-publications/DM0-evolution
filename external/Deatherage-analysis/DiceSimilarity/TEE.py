...
def mutation_line(feature, mutation):
    ...
    elif mutation[0] == "INS" or mutation[0] == "SUB":
...
def types_of_mutations(master_dict):
    ...
    for treatment in master_dict:
        ...
        for sample in master_dict[treatment]:
            for mutation in master_dict[treatment][sample]['total_mutations']:
                if mutation[0] == "SNP":
                    continue
                elif mutation[0] in ["MOB", "INS", "SUB"]:
    ...
    headings = ["Treament", "DEL", "AMP", "INS", "SUB", "Total MOB"] + list(sorted(set(mutation_type_dict["Total"]["MOB"])))
    print("\t".join(map(str, headings)))
    for condition in mutation_type_dict:
        if condition == "Total":
            continue
        body = [condition, len(mutation_type_dict[condition]["DEL"]), len(mutation_type_dict[condition]["AMP"]), len(mutation_type_dict[condition]["INS"]), len(mutation_type_dict[condition]["SUB"]), len(mutation_type_dict[condition]["MOB"])]
    ...
    body = ["Total", len(mutation_type_dict["Total"]["DEL"]), len(mutation_type_dict["Total"]["AMP"]), len(mutation_type_dict["Total"]["INS"]), len(mutation_type_dict[condition]["SUB"]), len(mutation_type_dict["Total"]["MOB"])]
    for mob_type in list(sorted(set(mutation_type_dict["Total"]["MOB"]))):
